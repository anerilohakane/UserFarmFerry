...
//   isVoiceModuleLoaded = true;
// } catch (e) {
//   isVoiceModuleLoaded = false;
// }

// // --- static fallback arrays (unchanged) ---
// const staticExploreCollection = [
//   { id: 1, name: 'Spices', image: 'https://images.unsplash.com/photo-1596040033229-a0b4b3f89ed7?w=200&h=200&fit=crop' },
//   { id: 2, name: 'Sweets', image: 'https://images.unsplash.com/photo-1571506165871-ee72a35bc9d4?w=200&h=200&fit=crop' },
//   { id: 3, name: 'Dairy', image: 'https://images.unsplash.com/photo-1628088062854-d1870b4553da?w=200&h=200&fit=crop' },
//   { id: 4, name: 'Grains', image: 'https://images.unsplash.com/photo-1586201375761-83865001e31c?w=200&h=200&fit=crop' },
// ];

// const staticNewDishes = [
//   { id: 1, name: 'Cucumber', price: '₹28', oldPrice: '₹30', unit: '500 g', image: 'https://images.unsplash.com/photo-1604977042399-0cifc0d7b8bc?w=300&h=300&fit=crop', isFavorite: false },
//   { id: 2, name: 'Mango', price: '₹150', oldPrice: '₹160', unit: '1 kg', image: 'https://images.unsplash.com/photo-1601493700631-2b16ec4b4716?w=300&h=300&fit=crop', isFavorite: true },
// ];

// const staticFreshness = [
//   { id: 1, name: 'Vegetables', image: 'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=200&h=200&fit=crop' },
//   { id: 2, name: 'Fruits', image: 'https://images.unsplash.com/photo-1619566636858-adf3ef46400b?w=200&h=200&fit=crop' },
//   { id: 3, name: 'Herbs', image: 'https://images.unsplash.com/photo-1509358271058-acd22cc93898?w=200&h=200&fit=crop' },
//   { id: 4, name: 'Dry fruits', image: 'https://images.unsplash.com/photo-1599599810769-bcde5a160d32?w=200&h=200&fit=crop' },
// ];

// const staticPantryItems = [
//   { id: 1, name: 'Juice, 24ct 24', image: 'https://images.unsplash.com/photo-1600271886742-f049cd451bba?w=200&h=200&fit=crop', category: 'Drinks' },
//   { id: 2, name: 'Dairy Snack of Milk', image: 'https://images.unsplash.com/photo-1563636619-e9143da7973b?w=200&h=200&fit=crop', category: 'Drinks' },
//   { id: 3, name: 'Breakfast Cereals', image: 'https://images.unsplash.com/photo-1621939514649-280e2ee25f60?w=200&h=200&fit=crop', category: 'Breakfast' },
//   { id: 4, name: 'Packaged Snacks', image: 'https://images.unsplash.com/photo-1621939505556-e72b213d2dbc?w=200&h=200&fit=crop', category: 'Snacks' },
// ];

// const staticCravingsItems = [
//   { id: 1, name: 'Tea, Coffee & More', image: 'https://images.unsplash.com/photo-1517668808822-9ebb02f2a0e6?w=200&h=200&fit=crop' },
//   { id: 2, name: 'Ice Cream & More', image: 'https://images.unsplash.com/photo-1563805042-7684c019e1cb?w=200&h=200&fit=crop' },
//   { id: 3, name: 'Sweet Cravings', image: 'https://images.unsplash.com/photo-1606312619070-d48b4cda81c9?w=200&h=200&fit=crop' },
// ];

// const staticSnacksItems = [
//   { id: 1, name: 'Biscuits & Cookies', image: 'https://images.unsplash.com/photo-1558961363-fa8fdf82db35?w=200&h=200&fit=crop', category: 'Snacks' },
//   { id: 2, name: 'Chips', image: 'https://images.unsplash.com/photo-1566478989037-eec170784d0b?w=200&h=200&fit=crop', category: 'Snacks' },
//   { id: 3, name: 'Munchies', image: 'https://images.unsplash.com/photo-1613919113640-25732ec5e61f?w=200&h=200&fit=crop', category: 'Snacks' },
// ];
...
//   // integration states
//   const [searchQuery, setSearchQuery] = useState('');
//   const [fetchedProducts, setFetchedProducts] = useState([]); // normalized
//   const [filteredProducts, setFilteredProducts] = useState([]);
//   const [allCategoryProducts, setAllCategoryProducts] = useState([]);
//   const [categories, setCategories] = useState([]);
...
//   // helpers
//   const isInWishlist = (id) => wishlistItems?.some(it => it && (it._id === id || it.id === id));
//   const isInCart = (id) => cartItems?.some(it => it && (it._id === id || it.id === id));

//   // ---------------- Fuzzy helpers (same as before) ----------------
//   const levenshteinDistance = (str1, str2) => {
//     const track = Array(str2.length + 1).fill(null).map(() =>
//       Array(str1.length + 1).fill(null));
...
//         const s = calculateSimilarity(qw, tw);
//         if (s > best) best = s;
//       });
//       if (best > 0.3) {
...
//       const searchableFields = [prod.name || '', prod.category || '', prod.farmer || '', prod.description || ''];
//       let bestScore = 0;
//       searchableFields.forEach(f => {
//         const score = getBestMatch(query, f);
...
//     // defensive: handle many possible field names
//     const id = p._id || p.id || p.productId || null;
//     const images = p.images || p.image || [];
//     const image = Array.isArray(images) ? (images[0]?.url || images[0]) : (typeof images === 'string' ? images : (p.image || 'https://via.placeholder.com/300'));
//     const price = (p.discountedPrice > 0 ? p.discountedPrice : p.price) ?? p.price ?? 0;
//     const originalPrice = p.price ?? p.originalPrice ?? 0;
//     const discount = p.offerPercentage ?? p.discount ?? 0;
//     const rating = p.averageRating ?? p.rating ?? 0;
//     const reviews = p.totalReviews ?? p.reviews ?? 0;
//     const farmer = p.supplierId?.businessName || p.farmer || '';
//     const category = p.categoryId?.name || p.category || '';
//     const stockQuantity = typeof p.stockQuantity === 'number' ? p.stockQuantity : (p.stock ?? 0);
//     const inStock = stockQuantity > 0;
//     return {
//       ...p,
//       _normalized: true,
//       id,
//       _id: id, // keep _id handy
//       image,
//       price,
//       originalPrice,
//       discount,
...
//       console.log('[GroceryHome] fetched products:', normalized.length);
//       return normalized;
//     } catch (err) {
//       console.warn('[GroceryHome] fetchProducts error', err?.response?.data || err?.message || err);
//       setFetchedProducts([]);
//       setFilteredProducts([]);
//       return [];
//     } finally {
...
//       const items = res?.data?.data?.products || res?.data?.products || [];
//       const normalized = items.map(normalizeProduct);
//       setAllCategoryProducts(normalized);
//       console.log('[GroceryHome] fetched all products:', normalized.length);
//       return normalized;
//     } catch (err) {
//       console.warn('[GroceryHome] fetchAllProducts error', err?.response?.data || err?.message || err);
//       return [];
//     } finally {
//       setLoadingSearch(false);
//     }
...
//       setCategories(cats);
//       console.log('[GroceryHome] fetched categories:', cats.length);
//       return cats;
//     } catch (err) {
//       console.warn('[GroceryHome] fetchCategories error', err?.response?.data || err?.message || err);
//       // fallback attempt
//       try {
//         const fallback = await categoriesAPI.getCategories();
//         const cats2 = fallback?.data?.data?.categories || fallback?.data?.categories || fallback?.data?.data || [];
//         setCategories(cats2.filter(c => !c.parent));
//         console.log('[GroceryHome] fetched categories fallback:', cats2.length);
//         return cats2;
//       } catch (err2) {
//         console.warn('[GroceryHome] fetchCategories fallback failed', err2?.message || err2);
//         setCategories([]);
//         return [];
//       } finally {
//         setLoadingCategories(false);
...
//   // ---------------- Cart & Wishlist ----------------
//   const handleAddToCart = async (product) => {
//     const prod = product?._raw ?? product;
//     if (!prod) {
//       Alert.alert('Error', 'Invalid product');
//       return;
//     }
//     const productId = prod._id || prod.id;
//     if (!prod.inStock && !(prod.stockQuantity > 0)) {
//       Alert.alert('Out of stock', `${prod.name || 'This item'} is currently out of stock`);
//       return;
//     }
//     if (!isInCart(productId)) {
//       try {
//         const res = await cartAPI.addToCart({ productId, quantity: 1 });
//         updateCartItems(res.data.data.cart.items);
//         console.log('[GroceryHome] added to cart', productId);
//       } catch (err) {
//         console.warn('[GroceryHome] addToCart error', err?.response?.data || err?.message || err);
...
//     if (isInWishlist(productId)) {
//       await removeFromWishlist(productId);
//       console.log('[GroceryHome] removed wishlist', productId);
//     } else {
//       await addToWishlist(prod);
//       console.log('[GroceryHome] added wishlist', productId);
//     }
//   };

//   // ---------------- Voice logic (same safe approach) ----------------
//   const onSpeechStart = () => {
//     setIsRecording(true);
//     setVoiceResults([]);
//     setRecordingTime(0);
...
//   const onSpeechEnd = () => cleanupRecording();
//   const onSpeechError = (e) => {
//     cleanupRecording();
//     Alert.alert('Speech Error', e?.message || 'Could not recognize speech. Please try again.');
//   };
//   const cleanupRecording = () => {
//     setIsRecording(false);
//     setRecordingTime(0);
...
//       setSearchQuery(text);
//       const suggestions = await fetchVoiceSearchSuggestions(text);
//       setVoiceResults(suggestions);
//       setShowVoiceResults(true);
//     }
//     cleanupRecording();
...
//   const requestMicrophonePermission = async () => {
//     if (!isVoiceModuleLoaded) return false;
//     if (Platform.OS === 'android') {
//       try {
...
//         productsAPI.getProducts({ limit: 100 }),
//       ]);
//       const cats = catsRes.status === 'fulfilled' ? (catsRes.value?.data?.data?.categories || catsRes.value?.data?.categories || catsRes.value?.data?.data || []) : categories;
//       const prods = prodsRes.status === 'fulfilled' ? (prodsRes.value?.data?.data?.products || prodsRes.value?.data?.products || []) : fetchedProducts;
//       if (query && query.trim()) {
//         const cands = cats.filter(c => c.name && getBestMatch(query, c.name) > 0.3).slice(0,5).map(c => c.name);
//         const pcds = prods.filter(p => (p.name && getBestMatch(query, p.name) > 0.3)).slice(0,5).map(p => p.name);
//         const all = [...new Set([...cands, ...pcds])];
//         return all.length ? all.slice(0,8) : [query];
//       } else {
//         const topCats = (cats.slice ? cats.slice(0,5).map(c => c.name) : []).filter(Boolean);
//         const topProds = (prods.slice ? prods.slice(0,3).map(p => p.name) : []).filter(Boolean);
//         return [...topCats, ...topProds].length ? [...topCats, ...topProds] : ['Search for products'];
//       }
//     } catch (err) {
//       return ['Try searching for products'];
//     } finally {
...
//       }, 1000);
//       setTimeout(async () => {
//         const suggestions = await fetchVoiceSearchSuggestions('');
//         setVoiceResults(suggestions);
//         setShowVoiceResults(true);
//         setIsRecording(false);
//       }, 1300);
...
//     if (!voiceInitialized) await initializeVoiceRecognition();
//     const hasPermission = await requestMicrophonePermission();
//     if (!hasPermission) {
...
//   useEffect(() => {
//     if (searchQuery && showVoiceResults) setShowVoiceResults(false);
//   }, [searchQuery]);
...
//   // search behavior (fuzzy across ALL products)
//   useEffect(() => {
//     let cancelled = false;
//     const performSearch = async () => {
...
//       }
//       setLoadingSearch(true);
//       let all = allCategoryProducts;
//       if (!all.length) all = await fetchAllProducts();
//       if (cancelled) return;
//       const res = fuzzySearch(all, searchQuery);
...
//   // ---------------- Derived UI arrays (prefer dynamic) ----------------
//   const derivedExploreCollection = (categories && categories.length > 0)
//     ? categories.slice(0,4).map((c,i) => ({
//         id: c._id || c.id || `cat-${i}`,
//         name: c.name || c.title || 'Category',
//         image: (typeof c.image === 'string' ? c.image : c.image?.url) || c.icon || 'https://via.placeholder.com/200',
//       }))
//     : staticExploreCollection;

//   // new dishes: prefer fetchedProducts (discounted first), else static
//   const derivedNewDishes = (() => {
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       // sort: discounted/offer first, then newest
//       const sorted = [...fetchedProducts].sort((a,b) => (b.discount || 0) - (a.discount || 0));
//       return sorted.slice(0,6).map((p, idx) => ({
//         id: p.id || p._id || `p-${idx}`,
//         name: p.name || 'Product',
//         price: `₹${p.price ?? 0}`,
//         oldPrice: p.originalPrice ? `₹${p.originalPrice}` : '',
//         unit: p.unit || p.weight || p.packSize || '',
//         image: p.image || 'https://via.placeholder.com/300',
//         isFavorite: isInWishlist(p._id || p.id),
//         _raw: p,
//       }));
//     }
//     return staticNewDishes;
//   })();

//   const derivedFreshness = (categories && categories.length > 0)
//     ? categories.slice(0,4).map((c,i) => ({
//         id: c._id || c.id || `f-${i}`,
//         name: c.name || 'Category',
//         image: (typeof c.image === 'string' ? c.image : c.image?.url) || 'https://via.placeholder.com/200',
//       }))
//     : staticFreshness;

//   const derivedPantryItems = (() => {
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       const pantryCandidates = fetchedProducts.filter(p =>
//         /drink|juice|beverage|milk|dairy|cereal|breakfast/i.test((p.category || '') + ' ' + (p.name || ''))
//       );
//       const source = pantryCandidates.length ? pantryCandidates : fetchedProducts;
//       return source.slice(0,4).map((p) => ({ id: p.id || p._id, name: p.name, image: p.image, category: p.category, _raw: p }));
//     }
//     return staticPantryItems;
//   })();

//   const derivedCravingsItems = (() => {
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       const cands = fetchedProducts.filter(p =>
//         /ice|sweet|dessert|coffee|tea|snack|ice cream|chocolate/i.test((p.name || '') + ' ' + (p.category || ''))
//       );
//       const src = cands.length ? cands : fetchedProducts;
//       return src.slice(0,3).map((p,i) => ({ id: p.id || p._id || `c-${i}`, name: p.name, image: p.image, _raw: p }));
//     }
//     return staticCravingsItems;
//   })();

//   const derivedSnacksItems = (() => {
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       const cands = fetchedProducts.filter(p =>
//         /snack|chips|biscuit|cookie|munch/i.test((p.name || '') + ' ' + (p.category || ''))
//       );
//       const src = cands.length ? cands : fetchedProducts;
//       return src.slice(0,3).map((p,i) => ({ id: p.id || p._id || `s-${i}`, name: p.name, image: p.image, category: p.category, _raw: p }));
//     }
//     return staticSnacksItems;
//   })();
...
//             <Feather name="mic" size={20} color="#4CAF50" />
//           </TouchableOpacity>
//         </View>
//       </View>
...
//         <View className="mb-6">
//           <Text className="text-lg font-bold px-4 mb-3">"Explore Our Grocery Collection"</Text>
//           <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-4">
//             <View className="flex-row gap-3">
//               {derivedExploreCollection.map((item) => (
//                 <View key={item.id} className="items-center w-20">
//                   <Image source={{ uri: item.image }} className="w-16 h-16 rounded-lg mb-1.5" />
...
//         {/* New Taste, New Deal */}
//         <View className="mb-6">
//           <Text className="text-lg font-bold px-4 mb-3">"New Taste, New Deal"</Text>
//           <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-4">
//             <View className="flex-row gap-3">
//               {derivedNewDishes.map((item) => (
//                 <View key={item.id} className="w-40 bg-white rounded-lg overflow-hidden border border-gray-200">
//                   <View className="relative">
...
//                       onPress={() => toggleWishlist(item._raw ?? item)}
//                     >
//                       <Feather name="heart" size={18} color={item.isFavorite || isInWishlist(item._raw?._id || item.id) ? "#FF0000" : "#666"} />
//                     </TouchableOpacity>
//                   </View>
//                   <View className="p-2.5">
//                     <Text className="text-sm font-medium mb-0.5">{item.name}</Text>
//                     <Text className="text-xs text-gray-500 mb-1.5">{item.unit}</Text>
//                     <View className="flex-row items-center justify-between">
//                       <View className="flex-row items-center gap-1.5">
//                         <Text className="text-sm font-semibold text-green-600">{item.price}</Text>
//                         <Text className="text-xs text-gray-400 line-through">{item.oldPrice}</Text>
//                       </View>
//                       <TouchableOpacity
//                         className="bg-green-500 rounded-full w-7 h-7 items-center justify-center"
//                         onPress={() => handleAddToCart(item._raw ?? item)}
//                       >
...
//         {/* Freshness handpicked for your family */}
//         <View className="mb-6">
//           <Text className="text-lg font-bold px-4 mb-3">"Freshness handpicked for your family"</Text>
//           <View className="px-4 flex-row flex-wrap justify-between">
//             {derivedFreshness.map((item) => (
//               <View key={item.id} className="items-center w-[23%] mb-4">
//                 <Image source={{ uri: item.image }} className="w-full aspect-square rounded-lg mb-1.5" />
//                 <Text className="text-xs text-center">{item.name}</Text>
//               </View>
//             ))}
//           </View>
...
//         {/* From crunchy bites to cool sips */}
//         <View className="mb-6">
//           <Text className="text-lg font-bold px-4 mb-3">"From crunchy bites to cool sips -- all your cravings in one place"</Text>
//           <View className="px-4 flex-row flex-wrap justify-between">
//             {derivedCravingsItems.map((item) => (
//               <View key={item.id} className="items-center w-[31%] mb-4">
//                 <Image source={{ uri: item.image }} className="w-full aspect-square rounded-lg mb-1.5" />
//                 <Text className="text-xs text-center leading-tight">{item.name}</Text>
//               </View>
//             ))}
//           </View>
//         </View>
...
//         <View className="absolute bottom-0 left-0 right-0 bg-white rounded-t-xl p-4 shadow-lg">
//           <View className="flex-row justify-between items-center mb-2">
//             <Text className="text-lg font-semibold">Voice Search Results</Text>
//             <TouchableOpacity onPress={() => setShowVoiceResults(false)}>
//               <MaterialIcons name="close" size={20} color="#666" />
//             </TouchableOpacity>
//           </View>
//           <View>
//             {voiceResults.map((r,i) => (
//               <TouchableOpacity key={`${r}-${i}`} className="py-2" onPress={() => handleVoiceResultSelect(r)}>
//                 <Text>{r}</Text>
//               </TouchableOpacity>
//             ))}
//           </View>
...
//     name: "Spices",
//     image:
//       "https://images.unsplash.com/photo-1596040033229-a0b4b3f89ed7?w=200&h=200&fit=crop",
//   },
//   {
//     id: 2,
//     name: "Sweets",
//     image:
//       "https://images.unsplash.com/photo-1571506165871-ee72a35bc9d4?w=200&h=200&fit=crop",
//   },
//   {
//     id: 3,
//     name: "Dairy",
...
//     unit: "500 g",
//     image:
//       "https://images.unsplash.com/photo-1604977042399-0cifc0d7b8bc?w=300&h=300&fit=crop",
//     isFavorite: false,
//   },
//   {
//     id: 2,
...
//     unit: "1 kg",
//     image:
//       "https://images.unsplash.com/photo-1601493700631-2b16ec4b4716?w=300&h=300&fit=crop",
//     isFavorite: true,
//   },
// ];
...
//     name: "Herbs",
//     image:
//       "https://images.unsplash.com/photo-1509358271058-acd22cc93898?w=200&h=200&fit=crop",
//   },
//   {
//     id: 4,
//     name: "Dry fruits",
...
//     image:
//       "https://images.unsplash.com/photo-1621939505556-e72b213d2dbc?w=200&h=200&fit=crop",
//     category: "Snacks",
//   },
// ];
...
//     name: "Sweet Cravings",
//     image:
//       "https://images.unsplash.com/photo-1606312619070-d48b4cda81c9?w=200&h=200&fit=crop",
//   },
// ];
...
//     name: "Munchies",
//     image:
//       "https://images.unsplash.com/photo-1613919113640-25732ec5e61f?w=200&h=200&fit=crop",
//     category: "Snacks",
//   },
// ];
...
//   const [subCategoriesMap, setSubCategoriesMap] = useState({}); // parentId -> [subcats]
//   const [freshnessSubcategories, setFreshnessSubcategories] = useState([]);
//   const [loadingProducts, setLoadingProducts] = useState(false);
//   const [loadingCategories, setLoadingCategories] = useState(false);
//   const [loadingSearch, setLoadingSearch] = useState(false);
...
//     wishlistItems?.some((it) => it && (it._id === id || it.id === id));
//   const isInCart = (id) =>
//     cartItems?.some((it) => it && (it._id === id || it.id === id));
...
//     const price =
//       (p.discountedPrice > 0 ? p.discountedPrice : p.price) ?? p.price ?? 0;
//     const originalPrice = p.price ?? p.originalPrice ?? 0;
//     const discount = p.offerPercentage ?? p.discount ?? 0;
//     const rating = p.averageRating ?? p.rating ?? 0;
//     const reviews = p.totalReviews ?? p.reviews ?? 0;
...
//     } catch (err) {
//       console.warn(
//         "[GroceryHome] fetchProducts error",
...
//     } catch (err) {
//       console.warn(
//         "[GroceryHome] fetchAllProducts error",
//         err?.response?.data || err?.message || err
//       );
//       return [];
//     } finally {
...
//   // fetch parent categories (top-level) and build subCategoriesMap
//   const fetchCategories = async () => {
//     setLoadingCategories(true);
//     try {
//       // fetch parent categories (parent=null)
//       const mainRes = await categoriesAPI.getCategories({ parent: "null" });
//       const mainCats =
//         mainRes?.data?.data?.categories ||
//         mainRes?.data?.categories ||
//         mainRes?.data?.data ||
//         [];

//       // fetch all categories to map children (fallback)
//       const allRes = await categoriesAPI.getCategories();
//       const allCats =
//         allRes?.data?.data?.categories ||
//         allRes?.data?.categories ||
//         allRes?.data?.data ||
//         [];

//       // Build map parentId -> subcats
//       const map = {};
//       // attempt to populate using server children property if present
//       mainCats.forEach((parent) => {
//         const pid = parent._id || parent.id;
//         // find direct children in allCats
//         const children = allCats.filter(
//           (c) =>
//             c.parent === pid ||
//             (c.parent && (c.parent._id === pid || c.parent === pid))
//         );
//         // fallback to nested fields on parent object if any
//         const nested =
//           parent.children ||
...
//     } catch (err) {
//       console.warn(
//         "[GroceryHome] fetchCategories error",
//         err?.response?.data || err?.message || err
//       );
//       setCategories([]);
//       setSubCategoriesMap({});
...
//   // fetch subcategories for "Freshness" using all parents (no forced slice to 4)
//   const fetchSubcategoriesForFreshness = async (parentCats = []) => {
//     if (!parentCats || parentCats.length === 0) {
//       setFreshnessSubcategories([]);
//       return [];
//     }

//     try {
//       const promises = parentCats.map(async (parent) => {
//         const parentId = parent._id || parent.id;
//         // try API call for children
//         try {
//           const res = await categoriesAPI.getCategories({ parent: parentId });
//           const subcats =
...
//               id: sc._id || sc.id || `sub-${sc.name}`,
//               name: sc.name || sc.title || sc.label || "Subcategory",
//               image:
//                 (typeof sc.image === "string" ? sc.image : sc.image?.url) ||
//                 sc.icon ||
...
//           }
//         } catch (err) {
//           // ignore per-parent API error
//         }

//         // fallback: nested children on parent object
//         const nested =
//           parent.children ||
//           parent.subcategories ||
//           parent.subCategories ||
...
//           return nested.map((sc) => ({
//             id: sc._id || sc.id || `sub-${sc.name}`,
//             name: sc.name || sc.title || "Subcategory",
//             image:
//               (typeof sc.image === "string" ? sc.image : sc.image?.url) ||
//               sc.icon ||
//               "https://via.placeholder.com/200",
...
//       // optional cap for performance — change CAP to Infinity to show ALL
//       const CAP = 8;
//       const final = deduped.slice(0, CAP);
//       setFreshnessSubcategories(final);
//       return final;
//     } catch (err) {
//       console.warn("[GroceryHome] fetchSubcategoriesForFreshness error", err);
//       setFreshnessSubcategories([]);
//       return [];
//     }
//   };
...
//       Alert.alert(
//         "Out of stock",
//         `${prod.name || "This item"} is currently out of stock`
//       );
//       return;
//     }
//     if (!isInCart(productId)) {
...
//     Alert.alert(
//       "Speech Error",
//       e?.message || "Could not recognize speech. Please try again."
//     );
//   };
//   const cleanupRecording = () => {
//     setIsRecording(false);
...
//       Alert.alert(
//         "Voice Error",
//         err?.message || "Failed to start voice recognition"
//       );
//       return false;
//     }
//   };
...
//           ? prodsRes.value?.data?.data?.products ||
//             prodsRes.value?.data?.products ||
//             []
//           : fetchedProducts;
//       if (query && query.trim()) {
//         const cands = cats
//           .filter((c) => c.name && getBestMatch(query, c.name) > 0.3)
//           .slice(0, 5)
//           .map((c) => c.name);
//         const pcds = prods
//           .filter((p) => p.name && getBestMatch(query, p.name) > 0.3)
//           .slice(0, 5)
//           .map((p) => p.name);
//         const all = [...new Set([...cands, ...pcds])];
//         return all.length ? all.slice(0, 8) : [query];
//       } else {
//         const topCats = (
//           cats.slice ? cats.slice(0, 5).map((c) => c.name) : []
//         ).filter(Boolean);
//         const topProds = (
//           prods.slice ? prods.slice(0, 3).map((p) => p.name) : []
//         ).filter(Boolean);
//         return [...topCats, ...topProds].length
//           ? [...topCats, ...topProds]
//           : ["Search for products"];
//       }
//     } catch (err) {
...
//   useEffect(() => {
//     (async () => {
//       // fetch categories first so we can get subcategories for freshness
//       const parents = await fetchCategories();
//       await fetchProducts();
//       if (parents && parents.length) {
//         await fetchSubcategoriesForFreshness(parents);
//       }
//     })();
//   }, []);
...
//   // ---------------- Derived UI arrays (prefer dynamic) ----------------
//   const derivedExploreCollection =
...
//     freshnessSubcategories && freshnessSubcategories.length > 0
//       ? freshnessSubcategories.map((c) => ({
//           id: c.id,
//           name: c.name,
//           image: c.image || "https://via.placeholder.com/200",
//           raw: c.raw || c,
//         }))
//       : categories && categories.length > 0
//         ? categories.map((c, i) => ({
...
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       const pantryCandidates = fetchedProducts.filter((p) =>
//         /drink|juice|beverage|milk|dairy|cereal|breakfast/i.test(
//           (p.category || "") + " " + (p.name || "")
//         )
//       );
//       const source = pantryCandidates.length
//         ? pantryCandidates
//         : fetchedProducts;
...
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       const cands = fetchedProducts.filter((p) =>
//         /ice|sweet|dessert|coffee|tea|snack|ice cream|chocolate/i.test(
//           (p.name || "") + " " + (p.category || "")
//         )
//       );
//       const src = cands.length ? cands : fetchedProducts;
//       return src.slice(0, 3).map((p, i) => ({
//         id: p.id || p._id || `c-${i}`,
//         name: p.name,
//         image: p.image,
//         _raw: p,
...
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       const cands = fetchedProducts.filter((p) =>
//         /snack|chips|biscuit|cookie|munch/i.test(
//           (p.name || "") + " " + (p.category || "")
//         )
//       );
//       const src = cands.length ? cands : fetchedProducts;
...
//                   activeOpacity={0.85}
//                   onPress={() => {
//                     // navigate to Subcategories screen and pass the selected category (raw if available)
//                     const categoryObj = item.raw || item;
...
//                 <View
//                   key={item.id}
//                   className="w-48 bg-white rounded-2xl overflow-hidden border border-gray-100"
//                 >
//                   {/* Image Container */}
//                   <View className="relative">
//                     <Image
...
//                     {/* Wishlist Button */}
//                     <TouchableOpacity
//                       className="absolute top-2 right-2 bg-white/90 rounded-full w-9 h-9 items-center justify-center shadow-sm"
//                       onPress={() => toggleWishlist(item._raw ?? item)}
//                     >
//                       <Feather
//                         name="heart"
...
//                         color={
//                           item.isFavorite ||
//                           isInWishlist(item._raw?._id || item.id)
//                             ? "#FF4444"
//                             : "#94A3B8"
//                         }
//                         fill={
...
//                           <Text className="text-xs text-gray-400">15min</Text>
//                         </View>
//                       </View>

//                       {/* Add to Cart Button */}
//                       <TouchableOpacity
//                         className="bg-green-500 rounded-xl w-9 h-9 items-center justify-center shadow-sm active:scale-95 transition-all"
//                         onPress={() => handleAddToCart(item._raw ?? item)}
//                       >
...
//                 activeOpacity={0.85}
//                 onPress={() => {
//                   // If this is a subcategory, navigate to Subcategories or Products for that subcategory
//                   const catObj = item.raw || item;
//                   // If subcategory page expects category param, pass it
//                   navigation.navigate("Subcategories", { category: catObj });
//                 }}
//               >
//                 <Image
//                   source={{ uri: item.image }}
...
//         <View className="mb-6">
//           <Text className="text-xl font-bold px-4 mb-3">
//             "From crunchy bites to cool sips -- all your cravings in one place"
//           </Text>
//           <View className="px-4 flex-row flex-wrap justify-between">
//             {derivedCravingsItems.map((item) => {
//               // use the raw normalized product if available, otherwise pass the lightweight object
//               const productPayload = item._raw ?? item;

//               return (
//                 <TouchableOpacity
//                   key={item.id}
//                   className="items-center w-[31%] mb-4"
//                   activeOpacity={0.85}
//                   onPress={() => {
//                     // Option A: go to Product Details screen
//                     // navigation.navigate('ProductDetails', { product: productPayload });

//                     // Option B: go straight to OrderSummary (Buy) with quantity 1
//                     navigation.navigate("OrderSummary", {
//                       items: [{ ...productPayload, quantity: 1 }],
//                     });
//                   }}
//                 >
//                   <Image
...
//                 key={`${r}-${i}`}
//                 className="py-2"
//                 onPress={() => handleVoiceResultSelect(r)}
//               >
//                 <Text>{r}</Text>
//               </TouchableOpacity>
//             ))}
...
const CustomAlert = ({ 
  ...
  const getBackgroundColor = () => {
    // All alerts now use the green color #16a34a
    return 'bg-[#16a34a]';
  };
  ...
  return (
    <Modal transparent visible={visible} animationType="fade">
      <View className="flex-1 bg-black/50 justify-center items-center p-6">
        ...
        >
          {/* Header */}
          <View className={`${getBackgroundColor()} p-4 items-center`}>
          ...
          {/* Buttons */}
          <View className={`flex-row border-t border-gray-200 ${showCancel ? '' : 'justify-center'}`}>
            {showCancel && (
              <TouchableOpacity 
            ...
            <TouchableOpacity 
              className={`py-4 ${showCancel ? 'flex-1' : 'px-8'}`}
              onPress={onConfirm}
            >
              <Text className="text-center font-semibold text-base text-[#16a34a]">
                {confirmText}
              </Text>
            </TouchableOpacity>
          </View>
...
const Toast = ({ visible, message, type = 'info', onHide }) => {
  ...
  const getBackgroundColor = () => {
    // All toasts now use the green color #16a34a
    return 'bg-[#16a34a]';
  };
  ...
  return (
    <Animated.View 
      ...
      className={`absolute top-12 left-4 right-4 ${getBackgroundColor()} rounded-xl p-4 flex-row items-center shadow-lg z-50`}
    >
...
// ... (rest of the static arrays remain the same)
const staticExploreCollection = [
  {
    id: 1,
    name: "Spices",
...
export default function GroceryHomeScreen() {
  ...
  const onSpeechError = (e) => {
    ...
    showAlert({
      title: "Speech Error",
      message: e?.message || "Could not recognize speech. Please try again.",
      type: "error",
    });
  };
  const cleanupRecording = () => {
  ...
  const handleMicrophonePress = async () => {
    ...
    if (!hasPermission) {
      showAlert({
        ...
        onConfirm: () => {
          // You can navigate to app settings here if needed
        },
      });
      return;
    }
  ...
  const showAddToCartConfirmation = (item) => {
    const product = item._raw ?? item;
    ...
    showAlert({
      ...
      cancelText: "Cancel",
      onConfirm: () => handleAddToCart(product),
      onCancel: () => {
        showToast("Item not added to cart", "info");
      }
    });
  };
  ...
  const handleAddToCart = async (product) => {
    ...
    if (!prod.inStock && !(prod.stockQuantity > 0)) {
      showAlert({
        title: "Out of Stock",
        message: `${prod.name || "This item"} is currently out of stock`,
        type: "warning",
      });
      return;
    }
    if (!isInCart(productId)) {
      try {
        const res = await cartAPI.addToCart({ productId, quantity: 1 });
        updateCartItems(res.data.data.cart.items);
        showToast(`${prod.name || "Item"} added to cart successfully! 🛒`, "success");
      } catch (err) {
        console.warn(
          "[GroceryHome] addToCart error",
          err?.response?.data || err?.message || err
                ...
                >
                  ...
                  <View className="p-3">
                    ...
                    <View className="flex-row items-center justify-between">
                      ...
                      {/* Add to Cart Button - Updated with green color */}
                      <TouchableOpacity
                        className="bg-[#16a34a] rounded-xl w-9 h-9 items-center justify-center shadow-sm active:scale-95 transition-all"
                        onPress={() => showAddToCartConfirmation(item)}
                      >
                        <Feather name="plus" size={18} color="white" />
                      </TouchableOpacity>
                    </View>
